...
def black_scholes_put_div(spxclose, strike, ttm, r, q, sigma):
    ...
    put = (spxclose * np.exp(-r * ttm) * si.norm.cdf(-d2, 0.0, 1.0) -
           strike * np.exp(-q * ttm) * si.norm.cdf(-d1, 0.0, 1.0))
...
# --------------------------------------------------
data['put'] = data.apply(lambda row: black_scholes_put_div(
